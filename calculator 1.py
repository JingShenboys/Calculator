...
def caluli():
    ...
    def f3():
       ...
       # baoyao = tkinter.IntVar()  # 将输入的注册名赋值给变量
        #baoyao.set(0)  # 将最初显示定为'example@python.com'
        #tkinter.Label(window_sign_up2, text='爆炒腰花(100g)：').place(x=10, y=170)  # 将`User name:`放置在坐标（10,10）。
        #entry_baoyao = tkinter.Entry(window_sign_up2, textvariable = baoyoa, width = 10)  # 创建一个注册名的`entry`，变量为`new_name`
        #entry_baoyao.place(x=80, y=170)  # `entry`放置在坐标（150,10）
        
        
        def f4():
            zaocan.set(mantou.get()*280+ mifan.get()*140 + shuijiao.get()*42 + dousha.get()*215+xianrou.get()*225
                       +jiucai * 260 + hanfan * 228 + qingtong * 64) ############要改的部分
    ...
    def f5():
        ...
        def f6():
            wucan.set(mantou.get()*280+ mifan.get()*140 + shuijiao.get()*42 + dousha.get()*215+xianrou.get()*225
                       +jiucai * 260 + hanfan * 228 + qingtong * 64)
